...
def run_backtest(backtester=None, start_date=START_DATE, end_date=END_DATE,plot_performance=True):
    ...
    inp.loc[inp.query(STRATEGY_PARAMS["condition"]).index, "isSPSEntry"] = True
    ...
    for i, time in enumerate(time_range):
        assert backtester.trades_entered_today <= backtester.config.max_positions_per_day, "Daily position cap exceeded"
        assert len(backtester.active_trades) <= backtester.config.max_positions, "Position cap exceeded"
        ...
        try:
            entry_signal = inp.loc[time, "isSPSEntry"]

...
class Order(abc.ABC, OptionStrategy):
    ...
    def short_description(self):
        if self.strategy_type == "Vertical Spread":
            long_leg = next(
                (leg for leg in self.legs if leg.position_side == "BUY"), None
            )
            short_leg = next(
                (leg for leg in self.legs if leg.position_side == "SELL"), None
            )
            return (
            ...
            put_short_leg = next(
                (
                    leg
                    for leg in self.legs
                    if leg.option_type == "PUT" and leg.position_side == "SELL"
                ),
                None,
            )
            call_long_leg = next(
                (
                    leg
                    for leg in self.legs
                    if leg.option_type == "CALL" and leg.position_side == "BUY"
                ),
                None,
            )
            call_short_leg = next(
                (
                    leg
                    for leg in self.legs
                    if leg.option_type == "CALL" and leg.position_side == "SELL"
                ),
                None,
            )
            return (
            ...
            call_leg = next(
                (leg for leg in self.legs if leg.option_type == "CALL"), None
            )
            ...
            lower_leg = next(
                (
                    ...
                    and leg.strike == min(leg.strike for leg in self.legs)
                ),
                None,
            )
            middle_leg = next(
                (
                    ...
                    and leg.strike == sorted(leg.strike for leg in self.legs)[1]
                ),
                None,
            )
            upper_leg = next(
                (
                    ...
                    for leg in self.legs
                    if leg.position_side == "BUY"
                    and leg.strike == max(leg.strike for leg in self.legs)
                ),
                None,
            )
            return (
            ...
            return f"Order ID: {self.order_id}, " f"Status: {self.order_status}"

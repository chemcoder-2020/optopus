...
from ._constants import Category, Imports, EXCHANGE_TZ, version
from .candles.cdl_pattern import ALL_PATTERNS, cdl_pattern
from .candles.cdl_z import cdl_z
from .candles.ha import ha
from .cycles.ebsw import ebsw
...
from .overlap.supertrend import supertrend  # Keep the first one
...
from .statistics.tos_stdevall import tos_stdevall
...
# from .trend.supertrend import supertrend # Remove duplicate
from .trend.tsignals import tsignals
from .trend.ttm_trend import ttm_trend
from .trend.vhf import vhf
from .trend.vortex import vortex
from .trend.xsignals import xsignals
...
from .volatility.hwc import hwc
...
from .volatility.thermo import thermo
from .volatility.true_range import true_range
from .volatility.ui import ui
...
from .utils import (
    ...
    cross,
    cross_value,
)  # Utility functions used as methods
...
class Strategy:
    ...
    def __post_init__(self):
        ...
        if self.ta is None:
            ...
            if not all([isinstance(_, dict) and len(_.keys()) > 0 for _ in self.ta]):
            ...
            not ta_is_list or self.total_ta() == 0
        ):  # Handle empty list or non-list case explicitly
            s = " - ta. Format is a list of dicts. Example: [{'kind': 'sma', 'length': 10}]"
            s += "\n       Check the indicator for the correct arguments if you receive this error."
            required_args.append(s)
...
class AnalysisIndicators(BasePandasObject):
    ...
    _adjusted = None
    _cores = cpu_count()
    _df = pd.DataFrame()  # Use pd.DataFrame explicitly
    _exchange = "NYSE"
    _time_range = "years"
    ...
    def cores(self, value: int) -> None:
        ...
        if value is not None and isinstance(value, int):
            # Check int(value) for comparison
            self._cores = int(value) if 0 <= int(value) <= cpus else cpus
        else:
            self._cores = cpus
    ...
    def _get_column(self, series):
        ...
        # Ok.  So it's a str.
            ...
            if series in df.columns:
                ...
                # Use regular string, not f-string if no variables needed
    ...
    def _post_process(self, result, **kwargs) -> Tuple[pd.Series, pd.DataFrame]:
        ...
        * Appends the result to main DataFrame
        ...
        if not isinstance(result, (pd.Series, pd.DataFrame)):
            if verbose:
                print(
                    "[X] Oops! The result was not a Series or DataFrame."
                )  # Not an f-string
            return self._df
        else:
    ...
    def strategy(self, *args, **kwargs):
        ...
        if mode["category"]:
            ...
            print("[X] Not an available strategy.")  # Not an f-string
            return None
        ...
        if use_multiprocessing:
            ...
            if verbose:
                _col_msg = "[i] No mulitproccessing (cores = 0)."  # Not an f-string
                if has_col_names:
                    _col_msg = "[i] No mulitproccessing support for 'col_names' option."  # Not an f-string
                print(_col_msg)

            if mode["custom"]:
                if Imports["tqdm"] and verbose:
                    pbar = tqdm(ta, "[i] Progress")  # Not an f-string
                    for ind in pbar:
                        params = (
                            ind["params"]
                            if "params" in ind and isinstance(ind["params"], tuple)
    ...
    # def supertrend(self, period=None, multiplier=None, mamode=None, drift=None, offset=None, **kwargs): # Duplicate removed
    #     high = self._get_column(kwargs.pop("high", "high"))
    #     low = self._get_column(kwargs.pop("low", "low"))
    #     close = self._get_column(kwargs.pop("close", "close"))
    ...
    def tsignals(
        ...
        **kwargs,
        ...
        if trend is None:
            return self._df  # Return original DataFrame if no trend provided
        else:
            result = tsignals(
                trend,
                asbool=asbool,

...
class CompositePipelineCondition(EntryConditionChecker):
    ...
    def __init__(self, pipeline: BaseComponent):
        """
        Args:
...
class PremiumMedianFilterCondition(BaseComponent):
    ...
    def should_enter(self, strategy, manager, time: pd.Timestamp) -> bool:
        ...
        # Initialize current median premiums tracking if needed
        if "current_median_premiums" not in manager.context:
            manager.context["current_median_premiums"] = []
        ...
        fluctuation = self.kwargs.get("fluctuation", 0.1)
        if len(manager.context["median_premiums"]) > window_size:
        ...
        try:
            ...
            mark_deviation = (
                abs((mark - current_median) / current_median)
                if current_median != 0
                else 0.0
            )

            bid_mark_deviation = abs((bid - mark) / mark) if mark != 0 else 0.0

            logger.debug(
                f"Median filter check: {mark_deviation:.2%} vs allowed {fluctuation:.2%}"
            )

            logger.debug(
                f"Median filter check (Bid-Mark Deviation): {bid_mark_deviation:.2%} vs allowed {fluctuation:.2%}"
            )

            # Store calculated median in context
            manager.context["current_median_premiums"].append(current_median)

            if len(manager.context["current_median_premiums"]) > window_size:
                manager.context["current_median_premiums"].pop(0)

            return mark_deviation <= fluctuation and bid_mark_deviation <= fluctuation
...
class CapitalRequirementCondition(BaseComponent):
    """
...
class MaxPositionLimitCondition(BaseComponent):
    """
    Checks if position limits are respected.

    Method:
        should_enter(strategy, manager, time) -> bool:
            Checks if the number of max active trades are within the configured limits.
    """
    ...
    def should_enter(
        ...
        return position_limit_ok


class DailyLimitCondition(BaseComponent):
    """
    Checks if position limits are respected.

    Method:
        should_enter(strategy, manager, time) -> bool:
            Checks if the number of daily trades are within the configured limits.
    """
    ...
    def should_enter(
        ...
        return daily_limit_ok


class WeeklyLimitCondition(BaseComponent):
    """
    Checks if position limits are respected.
    ...
    def should_enter(
        ...
        return weekly_limit_ok


class RORThresholdCondition(BaseComponent):
    """
...
class ConflictCondition(BaseComponent):
    """
    ...
    def __init__(self, check_closed_trades: bool = True):
        """
        Initialize the conflict condition checker.

        Args:
            check_closed_trades (bool): Whether to also check for conflicts with closed trades. Default is True.
        """
...
class TimeBasedEntryCondition(BaseComponent):
    """
...
class TrailingStopEntry(BaseComponent):
    def __init__(self, **kwargs):
        """
        Trailing stop entry condition checker (BaseComponent).
...
class DefaultEntryCondition(EntryConditionChecker):
    def __init__(self, **kwargs):
        ...
        self.pipeline = (
            ...
            * TimeBasedEntryCondition(
                allowed_days=self.kwargs.get(
                    "allowed_days", ["Mon", "Tue", "Wed", "Thu", "Fri"]
                ),
                allowed_times=self.kwargs.get("allowed_times", ["09:45-15:45"]),
                timezone=self.kwargs.get("timezone", "America/New_York"),
            )
            * MaxPositionLimitCondition()
            * DailyLimitCondition()
            * WeeklyLimitCondition
            * RORThresholdCondition()
            * ConflictCondition(
                check_closed_trades=self.kwargs.get("check_closed_trades", True)
            )
            * PremiumHampelFilterCondition(
                window_size=self.kwargs.get("window_size", 10),
                n_sigma=self.kwargs.get("n_sigma", 3),
                k=self.kwargs.get("k", 1.4826),
                max_iterations=self.kwargs.get("max_iterations", 5),
            ...
            * TrailingStopEntry(
                trailing_entry_direction=self.kwargs.get(
                    "trailing_entry_direction", "bullish"
                ),
                trailing_entry_threshold=self.kwargs.get("trailing_entry_threshold", 0),
                method=self.kwargs.get("method", "percent"),
                trailing_entry_reset_period=self.kwargs.get(
                    "trailing_entry_reset_period", None
                ),
            )
        )
    ...
    def should_enter(self, strategy, manager, time):
        logger.debug(f"Running DefaultEntryCondition at {time}")
        result = self.pipeline(strategy, manager, time)
        logger.debug(f"DefaultEntryCondition result: {result}")
        return result
